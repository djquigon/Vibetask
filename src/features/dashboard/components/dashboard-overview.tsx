  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid gap-3 md:grid-cols-4">
        {["Capture", "Plan my day", "Prioritize", "Brain dump"].map((item) => (
          <div
            key={item}
      ...
      <section className="rounded-md border border-[#25392f] bg-[#0d1b17] p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="font-mono text-xs text-[#50d678]">Thursday</span>
        </div>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <Metric title="Tasks overview" value="24" caption="total tasks" />
        <Metric title="Focus session" value="25:00" caption="deep work" />
        <Metric title="Today's progress" value="75%" caption="6 of 8 tasks" />
      </section>
    </div>
  );
}
